...
export default function MasterClasses() {
  ...
  useEffect(() => {
    if (data && data.data) { // Проверяем, есть ли данные в контексте
      ...
      const fetchData = async () => {
        setIsLoading(true);
        const result = await fetchGetCategory(`${endpoints.MASTERCLASSESBUSINESS}&pageNumber=${currentPage}&pageSize=${itemsPerPage}`);
        if (result) {
  ...
  }, [currentPage, data, endpoints.BEAUTYBUSINESS]);
      ...
      <section className="content">
        <div className="content__title-box">
          <h1 className="content__title">
            Мастер классы в <span className="content__city">{city === 1 ? "Полоцке" : "Новополоцке"}</span>
