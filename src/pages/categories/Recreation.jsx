...
export default function Recreation() {
  ...
  const currentPage = paginationState.recreationPage; 
  const currentPageNumbers = paginationState.recreationCurrentPageNumbers;
  ...
  useEffect(() => {
    if (data && data.data) {
      if (previousData !== data.data) {
        updatePage('recreationPage', 1);
        updateCurrentPageNumbers('recreation', []);
      ...
      const fetchData = async () => {
        setIsLoading(true);
        const result = await fetchGetCategory(`${endpoints.RECREACTION}&pageNumber=${currentPage}&pageSize=${itemsPerPage}`);
        if (result) {
      ...
      <section className="content">
        <div className="content__title-box" id='content'>
          <h1 className="content__title">
            Отдых в <span className="content__city">{cityName}</span>
        ...
        {isLoading ?
          ...
          <Content 
            ...
            setCurrentPage={(page) => { updatePage('recreationPage', page); }} 
            totalCount={totalCount}
            currentPageNumbers={currentPageNumbers}
            updateCurrentPageNumbers={(numbers) => updateCurrentPageNumbers('recreation', numbers)}
