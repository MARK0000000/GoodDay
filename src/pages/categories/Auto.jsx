  ...
  useEffect(() => {
    if (data && data.data) { // Проверяем, есть ли данные в контексте
      ...
      const fetchData = async () => {
        setIsLoading(true);
        const result = await fetchGetCategory(`${endpoints.AUTOBUSINESS}&pageNumber=${currentPage}&pageSize=${itemsPerPage}`);
        if (result) {
      ...
      <section className="content">
        <div className="content__title-box">
          <h1 className="content__title">
            Авто в <span className="content__city">{city === 1 ? "Полоцке" : "Новополоцке"}</span>
