...
export default function Education() {
  ...
  const currentPage = paginationState.educationPage; 
  const currentPageNumbers = paginationState.educationCurrentPageNumbers;
  ...
  useEffect(() => {
    if (data && data.data) {
      if (previousData !== data.data) {
        updatePage('educationPage', 1);
        updateCurrentPageNumbers('education', []);
      ...
      const fetchData = async () => {
        setIsLoading(true);
        const result = await fetchGetCategory(`${endpoints.EDUCATION}&pageNumber=${currentPage}&pageSize=${itemsPerPage}`);
        if (result) {
      ...
      <section className="content">
        <div className="content__title-box" id='content'>
          <h1 className="content__title">
            Обучение в <span className="content__city">{cityName}</span>
        ...
        {isLoading ?
          ...
          <Content 
            ...
            setCurrentPage={(page) => { updatePage('educationPage', page); }} 
            totalCount={totalCount}
            currentPageNumbers={currentPageNumbers}
            updateCurrentPageNumbers={(numbers) => updateCurrentPageNumbers('education', numbers)}
