...
export default function Equipment() {
  ...
  const currentPage = paginationState.equipmentPage; 
  const currentPageNumbers = paginationState.equipmentCurrentPageNumbers;
  ...
  useEffect(() => {
    if (data && data.data) {
      if (previousData !== data.data) {
        updatePage('equipmentPage', 1);
        updateCurrentPageNumbers('equipment', []);
      ...
      const fetchData = async () => {
        setIsLoading(true);
        const result = await fetchGetCategory(`${endpoints.EQUIPMENT}&pageNumber=${currentPage}&pageSize=${itemsPerPage}`);
        if (result) {
      ...
      <section className="content">
        <div className="content__title-box" id='content'>
          <h1 className="content__title">
            Техника в <span className="content__city">{cityName}</span>
        ...
        {isLoading ?
          ...
          <Content 
            ...
            setCurrentPage={(page) => { updatePage('equipmentPage', page); }} 
            totalCount={totalCount}
            currentPageNumbers={currentPageNumbers}
            updateCurrentPageNumbers={(numbers) => updateCurrentPageNumbers('equipment', numbers)}
