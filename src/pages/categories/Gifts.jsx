...
export default function Gifts() {
  ...
  const currentPage = paginationState.giftsPage; 
  const currentPageNumbers = paginationState.giftsCurrentPageNumbers;
  ...
  useEffect(() => {
    if (data && data.data) {
      if (previousData !== data.data) {
        updatePage('giftsPage', 1);
        updateCurrentPageNumbers('gifts', []);
      ...
      const fetchData = async () => {
        setIsLoading(true);
        const result = await fetchGetCategory(`${endpoints.GIFTS}&pageNumber=${currentPage}&pageSize=${itemsPerPage}`);
        if (result) {
      ...
      <section className="content">
        <div className="content__title-box" id='content'>
          <h1 className="content__title">
            Подарки в <span className="content__city">{cityName}</span>
        ...
        {isLoading ?
          ...
          <Content 
            ...
            setCurrentPage={(page) => { updatePage('giftsPage', page); }} 
            totalCount={totalCount}
            currentPageNumbers={currentPageNumbers}
            updateCurrentPageNumbers={(numbers) => updateCurrentPageNumbers('gifts', numbers)}
